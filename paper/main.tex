...
only --- not the backend. The currently shipped values are
\texttt{LANCZOS}, \texttt{LANCZOS\_SELECTIVE}, \texttt{LANCZOS\_NO\_ORTHO},
\texttt{BLOCK\_LANCZOS}, \texttt{KRYLOV\_SCHUR},
...
\texttt{BICG}, \texttt{ARPACK\_SM}, \texttt{ARPACK\_LM},
\texttt{ARPACK\_SHIFT\_INVERT}, \texttt{ARPACK\_ADVANCED},
\texttt{FULL}, \texttt{OSS}, \texttt{SCALAPACK},
\texttt{SCALAPACK\_MIXED}, \texttt{FTLM}, \texttt{LTLM}, \texttt{HYBRID},
\texttt{mTPQ}, and \texttt{cTPQ}.
...
\begin{table}[t]
\centering
\caption{Capability matrix of the five-axis dispatcher.
Every (\checkmark) cell has at least one regression test in the
shipped 150 + 197 test suite. \texttt{LOBPCG\_GPU} is currently
implemented by aliasing to the GPU Davidson kernel.}
...
\texttt{LANCZOS\_SELECTIVE}       & \checkmark & --         & \checkmark & \checkmark & \checkmark \\
\texttt{LANCZOS\_NO\_ORTHO}       & \checkmark & --         & \checkmark & \checkmark & \checkmark \\
\texttt{BLOCK\_LANCZOS}           & \checkmark & \checkmark & --         & \checkmark & \checkmark \\
\texttt{KRYLOV\_SCHUR}            & \checkmark & \checkmark & --         & \checkmark & \checkmark \\
\texttt{BLOCK\_KRYLOV\_SCHUR}     & \checkmark & \checkmark & --         & \checkmark & \checkmark \\
\texttt{DAVIDSON}                 & \checkmark & \checkmark & --         & \checkmark & \checkmark \\
\texttt{LOBPCG}                   & \checkmark & \checkmark$^\dagger$ & --         & \checkmark & \checkmark \\
\texttt{CHEBYSHEV\_FILTERED}      & \checkmark & --         & --         & \checkmark & --         \\
\texttt{SHIFT\_INVERT[\_ROBUST]}  & \checkmark & --         & --         & \checkmark & --         \\
\texttt{IRL} / \texttt{TRL} / \texttt{BICG} & \checkmark & -- & --         & \checkmark & --         \\
\texttt{ARPACK\_*}                & \checkmark & --         & --         & \checkmark & \checkmark \\
\texttt{FULL} / \texttt{OSS}      & \checkmark & \checkmark$^{\ddagger}$ & --   & \checkmark$^{\ddagger}$ & --         \\
\texttt{SCALAPACK[\_MIXED]}       & --         & --         & \checkmark & \checkmark$^{\ddagger}$ & --         \\
\texttt{FTLM}                     & \checkmark & \checkmark & \checkmark & \checkmark & \checkmark \\
\texttt{LTLM} / \texttt{HYBRID}   & \checkmark & --         & --         & \checkmark & \checkmark \\
...
\par\smallskip
\footnotesize
$^\dagger$ \texttt{LOBPCG\_GPU} aliases the GPU Davidson kernel.
$^{\ddagger}$ Dense kernels auto-split Sz sectors when applicable.
...
low $T$ but at higher cost per sample. Our \texttt{HYBRID} method
auto-crosses over from LTLM at low $T$ to FTLM at high $T$ at a
user-controllable boundary.
...
p.num_eigenvalues = 4
p.tolerance       = 1e-12
p.use_fixed_sz    = True; p.n_up = 6
p.use_gpu         = qed.has_cuda_build()
p.use_symmetry    = True

res = qed.exact_diagonalization_core(
    op, qed.DiagonalizationMethod.KRYLOV_SCHUR, p,
)
...
auto res = ed_dispatch::exact_diagonalization_from_directory(
    "./chain12", DiagonalizationMethod::KRYLOV_SCHUR, p);
...
Every flag on \texttt{EDParameters} is exposed through
\texttt{./ED}. The Phase~8 ScaLAPACK auto-block-size and explicit
override are wired through:

\begin{lstlisting}[language=bash]
./ED ./chain24 --method=SCALAPACK \
              --eigenvalues=8 \
              --scalapack-nprow=4 --scalapack-npcol=4 \
              --scalapack-block-size-auto      # default
./ED ./chain24 --method=SCALAPACK \
              --scalapack-block-size=128       # implicit auto-off

mpiexec -n 16 ./ED ./chain24 --method=LANCZOS_MPI \
              --symm --tolerance=1e-12
\end{lstlisting}
